...
import {
    ...
    AddLabor,
    GetLabors,
} from '../../store/client-labor/client-labor.action';
import { selectGetLabor, selectLoadingStatus } from '../../store/client-labor/client-labor.selector';
import { Observable } from 'rxjs';
import { HttpClient } from '@angular/common/http';
import { API } from '../API';
import { filter, switchMap, take, tap, map, delay } from 'rxjs/operators';
...
export class LaborService {
    ...
    getLaborById$(clientId: string): Observable<ILabor> {
        const sourceLabor$: Observable<ILabor> = this.store$.select(selectGetLabor).pipe(delay(100));
    ...
    fetchAndSave$(laborId: string): Observable<ILabor> {
        return this.store$.pipe(
            ...
            switchMap(() => {
                ...
                return this.httpClient.get<ILabor[]>(`${this.config.LABOR_URL}`).pipe(
                    tap((labors: ILabor[]) => this.store$.dispatch(new GetLabors(labors))),
                    map((labors: ILabor[]) => {
                        labors.filter(({ id }: ILabor) => id === laborId);
                        return labors[0];
                    }),
                    tap((labor: ILabor) => {
    ...
    addLabor$(form: ILabor): void {
        this.httpClient
            .post(this.config.LABOR_URL, form)
            .pipe(
                map((labor: ILabor) => {
